...
class DatasetStatistic(object):
    ...
    def get_taxonomy_file(self):
        ...
        :return:
        """
        file = codecs.open(os.path.join(self.config.data.data_dir, self.config.data.hierarchy), 'r', encoding='utf-8')
        data = file.readlines()
        file.close()
        hierarcy_dict = dict()
        for line in data:
        ...
        print(output_lines)
        file = codecs.open(os.path.join(self.config.data.data_dir, self.config.data.hierarchy), 'w', encoding='utf-8')
    ...
    def get_hierar_relations_with_name(taxo_file_dir):
        ...
        label_vocab = []
        f = open(taxo_file_dir, 'r', encoding='utf-8')
        ...
        for sample in data:
            ...
            for label in sample_label:
                ...
                if label in self.init_prior_prob_dict.keys():
                    ...
                    if label in ["CS", "Medical", "Civil", "ECE",
                                 'biochemistry', 'MAE', 'Psychology']:
...
if __name__ == '__main__':
    ...
    #gcn-rcv1-v2.json label-classification.json
    configs = Configure(config_json_file='./config/gcn-wos.json')
    ...
    with open(os.path.join(configs.data.data_dir, configs.data.prob_json), 'w', encoding='utf-8') as json_file:
        json_str = json.dumps(train_probs, ensure_ascii=False)
