...
class Trainer(object):
    ...
    def update_lr(self):
        ...
        for param in self.optimizer.param_groups:
            param['lr'] = torch.max(torch.tensor(param['lr'] * self.config.train.optimizer.lr_decay), torch.tensor(0.0001))
    ...
    def run(self, data_loader, epoch, stage, mode='TRAIN'):
        ...
        for batch in tqdm.tqdm(data_loader):
            logits, feature, R = self.model(batch)
            ...
            loss, loss1, loss2, loss3 = self.criterion(logits,
                                  batch['label'].to(self.config.train.device_setting.device),
                                  recursive_constrained_params,
                                  feature, R)
            loss1, loss2, loss3 = loss1.item(), loss2.item(), loss3.item()
            ...
            predict_probs.extend(logits.cpu().tolist())
            target_labels.extend(batch['label_list'])
            
        total_loss = total_loss / num_batch
        org_loss = org_loss / num_batch
        sim = sim / num_batch
        recur = recur / num_batch
        if mode == 'EVAL':
            ...
            logger.info("%s performance at epoch %d --- Precision: %f, "
                        "Recall: %f, Micro-F1: %f, Macro-F1: %f, Loss: %f, orgloss: %f,\
                            simloss: %f, recur_loss: %f.\n"
                        % (stage, epoch,
                           metrics['precision'], metrics['recall'], metrics['micro_f1'], metrics['macro_f1'],
                           total_loss, org_loss, sim, recur))
            return metrics
