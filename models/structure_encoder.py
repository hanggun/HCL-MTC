...
class StructureEncoder(nn.Module):
    def __init__(self,
        ...
        for p in self.hierarchy_prob.keys():#有多少节点
            if p == 'Root':
                continue
            for c in self.hierarchy_prob[p].keys():
                ...
                #parent -> child = hierarchy_prob
                self.node_prob_from_parent[int(self.label_map[c])][int(self.label_map[p])] = 1.0
        ...
        node_prob_from_child = self.node_prob_from_child
        node_prob_from_parent = self.node_prob_from_parent
        #Relu(in_+out_+loop_) batch, N, in_dim
    ...
    def forward(self, inputs, label_map):
        return self.model(inputs, label_map)
